...
def uploadToSDB(array,ondaName,sdb):
...
def downloadWFDB(onda,carpeta,sdb,ondaName):
  signalII = None
  try:
    print("checking: " + ondaName)
    sig, fields = wfdb.srdsamp(onda,pbdir='mimic3wdb/matched/'+carpeta, sampto=1)
    print("checked: " + ondaName)
    channel = fields['signame'].index("II")
    if(channel!=None):
      print("downloading: " + ondaName)
      sig = wfdb.rdsamp(onda,pbdir='mimic3wdb/matched/'+carpeta, channels = [channel]).p_signals
