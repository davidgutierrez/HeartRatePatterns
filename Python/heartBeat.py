#Code from: https://github.com/paulvangentcom/heartrate_analysis_python
...
def rolmean(hrdata, hrw, fs):
	...
	rol_mean = np.mean(rollwindow(hrarr, int(hrw*fs)), axis=1)
        lenhrarr = len(hrdata)
        lenrol_mean = len(rol_mean)
        lendiff = lenhrarr-lenrol_mean
	ln = np.array([avg_hr for i in range(0,abs(lendiff)/2)])
...
def filtersignal(hrdata, cutoff=4, fs, order=5):
